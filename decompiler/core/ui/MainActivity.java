...
public class MainActivity extends JFrame {
  ...
  private void initVariable() {
    setTitle(EasyI18N.getI18nString("哥斯拉   V%s by: BeichenDream Github:https://github.com/BeichenDream/Godzilla", new Object[] { "4.01" }));
    ...
    this.splitPane.setLeftComponent(new JScrollPane((Component)this.shellGroupTree));
    this.splitPane.setRightComponent(this.shellViewScrollPane = new JScrollPane((Component)this.shellView));
  ...
  private void removeShellMenuItemClick(ActionEvent e) {
    String[] shellIds = getSlectedShellId();
    if (shellIds.length > 0) {
      int n = GOptionPane.showConfirmDialog(getMainActivityFrame(), String.format(EasyI18N.getI18nString("确定删除id在 %s 的shell吗?"), new Object[] { Arrays.toString((Object[])shellIds) }), "警告", 0);
      if (n == 0) {
        for (int i = 0; i < shellIds.length; i++) {
          String shellId = shellIds[i];
          String shshellInfo = Db.getOneShell(shellId).toString();
          Log.log("removeShell status:%s  -> %s", new Object[] { Boolean.valueOf((Db.removeShell(shellId) > 0)), shshellInfo });
        } 
  ...
  private void interactCacheMenuItemClick(ActionEvent e) {
    ...
    try {
      if ((new File(String.format("%s/%s/cache.db", new Object[] { "GodzillaCache", shellId }))).isFile()) {
  ...
  public void refreshShellView() {
    ...
    this.statusLabel.setText(String.format(EasyI18N.getI18nString("共有%d组 所有成员数:%d 当前组是:%s 当前组成员数:%d "), new Object[] { Integer.valueOf(Db.getAllGroup().size()), Integer.valueOf(Db.getAllShell().size() - 1), this.currentGroup, Integer.valueOf(rowsVector.size()) }));
  }
