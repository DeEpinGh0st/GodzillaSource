...
public class ShellDatabasePanel extends JPanel {
  ...
  public ShellDatabasePanel(ShellEntity shellEntity) {
    ...
    this.commandpane = new JScrollPane((Component)this.sqlCommand);
    ...
    GBC gbcright1 = (new GBC(2, 0, 7, 1)).setFill(1).setWeight(1.0D, 0.7D).setInsets(0, 7, 0, 0);
    ...
    GBC gbcright2_6 = (new GBC(7, 1, 1, 1)).setFill(2).setWeight(1.0D, 0.0D);
    ...
    GBC gbcright4_1 = (new GBC(2, 3, 7, 1)).setFill(2).setWeight(1.0D, 0.0D).setInsets(0, 7, 0, 0);
    ...
    add(this.currentDbLabel, gbcright2_3);
    add(this.currentDbTextField, gbcright2_4);

    
    add(this.sql_listLabel, gbcright2_5);
    add(this.commonsql, gbcright2_6);

    
    add(this.commandpane, gbcright3);

    
    add(this.dbsetButton, gbcright2_7);
  ...
  private void fillDbListByDatabase() {
    ...
    String sqlString = (String)DatabaseSql.sqlMap.get(String.format("%s-getAllDatabase", new Object[] { this.dbInfo.getDbType().toLowerCase() }));
    if (sqlString != null) {
      this.sqlCommand.setText(sqlString);
      String result = execSql("select", sqlString);
      if (showData(result) && 
        this.dataView.getModel().getColumnCount() == 1) {
        
        Vector<Vector> rows = this.dataView.getModel().getDataVector();

        
        for (int i = 0; i < rows.size(); i++) {
          Vector<E> row = rows.get(i);
          this.dblist.AddNote(row.get(0).toString());
        } 
      } 
    } else {
      
      Log.error(String.format("Fill Database Fail! NO SQL %s", new Object[] { this.dbInfo.getDbType() }));
    } 
  }
  private void fillDbListByTable(String databaseName) {
    this.currentDbTextField.setText(databaseName);
    String sqlString = (String)DatabaseSql.sqlMap.get(String.format("%s-getTableByDatabase", new Object[] { this.dbInfo.getDbType().toLowerCase() }));
    if (sqlString != null) {
      sqlString = formatSql(sqlString, (String)null);
      this.sqlCommand.setText(sqlString);
      String result = execSql("select", sqlString);
      if (showData(result) && 
        this.dataView.getModel().getColumnCount() == 1) {
        ...
        for (int i = 0; i < rows.size(); i++) {
          ...
          this.dblist.AddNote(String.format("%s/%s", new Object[] { databaseName, row.get(0) }));
        } 
      } 
    } else {
      
      Log.error(String.format("Fill Table Fail! NO SQL %s", new Object[] { this.dbInfo.getDbType() }));
    } 
  }
  private void fillDataviewByDT(String databaseName, String tableName) {
    this.currentDbTextField.setText(databaseName);
    String sqlString = (String)DatabaseSql.sqlMap.get(String.format("%s-getTableDataByDT", new Object[] { this.dbInfo.getDbType().toLowerCase() }));
    if (sqlString != null) {
      sqlString = formatSql(sqlString, tableName);
      this.sqlCommand.setText(sqlString);
      String result = execSql("select", sqlString);
      showData(result);
    } else {
      Log.error(String.format("Fill TableData Fail! NO SQL %s", new Object[] { this.dbInfo.getDbType() }));
    } 
  }
  ...
  private void execButtonClick(ActionEvent actionEvent) {
    ...
    if (execSql != null && execSql.trim().length() > 0) {
      String result = execSql(execType, execSql);
      showData(result);
    } else {
  ...
  private void countTableMenuItemClick(ActionEvent e) {
    ...
    if (s.length == 2) {
      ...
      String sqlString = (String)DatabaseSql.sqlMap.get(String.format("%s-getCountByDT", new Object[] { this.dbInfo.getDbType().toLowerCase() }));
      if (sqlString != null) {
        sqlString = formatSql(sqlString, s[1]);
        this.sqlCommand.setText(sqlString);
        String result = execSql("select", sqlString);
        showData(result);
      } else {
