  ...
  extends JFrame
{
  ...
  public ShellSetting(String id, String defaultGroup) {
    ...
    lrSplitPane.setTopComponent(new JScrollPane((Component)this.leftTextArea));
    lrSplitPane.setBottomComponent(new JScrollPane((Component)this.rightTextArea));
    
    reqSplitPane.setTopComponent(new JScrollPane((Component)this.headersTextArea));
  ...
  private void testButtonClick(ActionEvent actionEvent) {
    if (updateTempShellEntity()) {
      if (this.shellContext.initShellOpertion()) {
        ...
        Log.log(String.format("CloseShellState: %s\tShellId: %s\tShellHash: %s", new Object[] { Boolean.valueOf(this.shellContext.getPayloadModule().close()), this.shellContext.getId(), Integer.valueOf(this.shellContext.hashCode()) }), new Object[0]);
      } else {
