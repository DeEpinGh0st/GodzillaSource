...
public class ShellEntity
{
  ...
  public boolean setEnv(String key, String value) {
    if (ApplicationContext.isOpenC("isSuperLog")) {
      Log.log(String.format("updateShellEnv id:%s key:%s value:%s", new Object[] { getId(), key, value }), new Object[0]);
    }
    if (existsSetingKey(key)) {
      String updateShellEnvSql = "UPDATE shellEnv set value=? WHERE shellId=? and key=?";
      PreparedStatement preparedStatement1 = Db.getPreparedStatement(updateShellEnvSql);
      try {
        preparedStatement1.setString(1, value);
        preparedStatement1.setString(2, getId());
        preparedStatement1.setString(3, key);
        int affectNum = preparedStatement1.executeUpdate();
    ...
    String updateSetingSql = "INSERT INTO shellEnv (\"shellId\",\"key\", \"value\") VALUES (?, ?, ?)";
  ...
  public void setGroup(String groupId) {
    setEnv("ENV_GROUP_ID", groupId);
  }
  public String getGroup() {
    return getEnv("ENV_GROUP_ID", "/");
  }
