...
public class PEParser
{
  public static PE parse(InputStream is) throws IOException {
    DataReader dr = new DataReader(is); Throwable throwable = null; 
    try { return read(dr); } catch (Throwable throwable1) { throwable = throwable1 = null; throw throwable1; }
    finally { $closeResource(throwable, dr); }
  ...
  public static PE parse(File file) throws IOException {
    FileInputStream is = new FileInputStream(file); Throwable throwable = null; 
    try { DataReader dr = new DataReader(is); Throwable throwable1 = null; }
    catch (Throwable throwable1) { throwable = throwable1 = null; throw throwable1; }
    finally { $closeResource(throwable, is); }
  ...
  public static PE read(IDataReader dr) throws IOException {
    ...
    if (pe.getDosHeader().getAddressOfNewExeHeader() == 0 || pe
      .getDosHeader().getAddressOfNewExeHeader() > 8192) {
      return pe;
    }
    ...
    while ((entry = findNextEntry(pe, dr.getPosition())) != null) {
      if (entry.isSection) {
        readSection(pe, entry, dr); continue;
      }  if (entry.isDebugRawData) {
        readDebugRawData(pe, entry, dr); continue;
      } 
  ...
  public static DOSHeader readDos(IDataReader dr) throws IOException {
    ...
    for (int j = 0; j < reserved2.length; j++) {
      reserved2[j] = dr.readWord();
    }
  ...
  public static SectionTable readSectionHeaders(PE pe, IDataReader dr) throws IOException {
    ...
    for (int j = 0; j < sorted.length; j++) {
      virtualAddress[j] = sorted[j].getVirtualAddress();
      pointerToRawData[j] = sorted[j].getPointerToRawData();
    } 
  ...
  public static DataEntry findNextEntry(PE pe, int pos) {
    ...
    for (int i = 0; i < ns; i++) {
      ...
      if (sh.getSizeOfRawData() > 0 && sh
        .getPointerToRawData() >= pos && (de.pointer == 0 || sh
        .getPointerToRawData() < de.pointer)) {
    ...
    for (int j = 0; j < dc; j++) {
      ImageDataDirectory idd = pe.getOptionalHeader().getDataDirectory(j);
      if (idd.getSize() > 0) {
  ...
  private static BoundImport readBoundImport(IDataReader dr) throws IOException {
    ...
    if (bi.getTimestamp() == 0L && bi.getOffsetToModuleName() == 0 && bi
      .getNumberOfModuleForwarderRefs() == 0) {
      return null;
    }
    return bi;
  }
