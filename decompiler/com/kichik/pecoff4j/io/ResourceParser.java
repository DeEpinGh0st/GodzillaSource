...
public class ResourceParser
{
  ...
  public static IconImage readIconImage(IDataReader dr, int bytesInRes) throws IOException {
    ...
    else if (ii.getHeader().getBitCount() <= 8) {
  ...
  public static VersionInfo readVersionInfo(IDataReader dr) throws IOException {
    ...
    alignDataReader(dr);
    vi.setFixedFileInfo(readFixedFileInfo(dr));
    ...
    while (true) {
      ...
      if ("VarFileInfo".equals(key)) {
        ...
        vi.setVarFileInfo(readVarFileInfo(dr, initialPos, length, valueLength, type, key, padding)); continue;
