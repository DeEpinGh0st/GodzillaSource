  ...
  extends FlatAbstractIcon
{
  ...
  protected void paintIcon(Component c, Graphics2D g) {
    ...
    if (this.chevron) {
      ...
      Path2D path = FlatUIUtils.createPath(false, new double[] { 1.0D, 0.0D, 5.0D, 4.0D, 9.0D, 0.0D });
      g.setStroke(new BasicStroke(1.0F));
      g.draw(path);
    } else {
      
      g.fill(FlatUIUtils.createPath(new double[] { 0.5D, 0.0D, 5.0D, 5.0D, 9.5D, 0.0D }));
    } 
  }
}
