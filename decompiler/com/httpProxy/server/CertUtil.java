...
public class CertUtil {
  ...
  public static PrivateKey loadPriKey(String path) throws Exception {
    return loadPriKey(Files.readAllBytes(Paths.get(path, new String[0])));
  }
  ...
  public static String getSubject(InputStream inputStream) throws Exception {
    ...
    return IntStream.rangeClosed(0, tempList.size() - 1)
      .<CharSequence>mapToObj(i -> (String)tempList.get(tempList.size() - i - 1)).collect(Collectors.joining(", "));
  }
  ...
  public static X509Certificate genCert(String issuer, PrivateKey caPriKey, Date caNotBefore, Date caNotAfter, PublicKey serverPubKey, String... hosts) throws Exception {
    String subject = Stream.<String>of(issuer.split(", ")).map(item -> { String[] arr = item.split("="); return "CN".equals(arr[0]) ? ("CN=" + hosts[0]) : item; }).collect(Collectors.joining(", "));



    
    JcaX509v3CertificateBuilder jv3Builder = new JcaX509v3CertificateBuilder(new X500Name(issuer), BigInteger.valueOf(System.currentTimeMillis() + (long)(Math.random() * 10000.0D) + 1000L), caNotBefore, caNotAfter, new X500Name(subject), serverPubKey);
  ...
  public static X509Certificate genCACert(String subject, Date caNotBefore, Date caNotAfter, KeyPair keyPair) throws Exception {
    JcaX509v3CertificateBuilder jv3Builder = new JcaX509v3CertificateBuilder(new X500Name(subject), BigInteger.valueOf(System.currentTimeMillis() + (long)(Math.random() * 10000.0D) + 1000L), caNotBefore, caNotAfter, new X500Name(subject), keyPair.getPublic());
