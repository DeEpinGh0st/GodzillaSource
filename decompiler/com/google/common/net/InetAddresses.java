...
public final class InetAddresses
{
  ...
  private static byte[] textToNumericFormatV6(String ipString) {
    ...
    for (int i = 1; i < parts.size() - 1; i++) {
      if (((String)parts.get(i)).length() == 0) {
        if (skipIndex >= 0) {
          return null;
        }
        skipIndex = i;
    ...
    if (skipIndex >= 0) {
      ...
      if (((String)parts.get(0)).length() == 0 && --partsHi != 0) {
        return null;
      }
      if (((String)Iterables.getLast(parts)).length() == 0 && --partsLo != 0) {
        return null;
      }
    }
    else {
    ...
    if ((skipIndex >= 0) ? (partsSkipped >= 1) : (partsSkipped == 0)) {
      ...
      ByteBuffer rawBytes = ByteBuffer.allocate(16); try {
        int j;
        for (j = 0; j < partsHi; j++) {
          rawBytes.putShort(parseHextet(parts.get(j)));
        }
        for (j = 0; j < partsSkipped; j++) {
          rawBytes.putShort((short)0);
        }
        for (j = partsLo; j > 0; j--) {
          rawBytes.putShort(parseHextet(parts.get(parts.size() - j)));
        }
      } catch (NumberFormatException ex) {
        return null;
      } 
