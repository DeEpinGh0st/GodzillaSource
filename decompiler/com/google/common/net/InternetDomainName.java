...
public final class InternetDomainName
{
  ...
  InternetDomainName(String name) {
    ...
    Preconditions.checkArgument(validateSyntax((List<String>)this.parts), "Not a valid domain name: '%s'", name);
  ...
  private int findSuffixOfType(Optional<PublicSuffixType> desiredType) {
    ...
    for (int i = 0; i < partsSize; i++) {
      ...
      if (matchesType(desiredType, 
          Optional.fromNullable(PublicSuffixPatterns.EXACT.get(ancestorName)))) {
        return i;
      }
  ...
  private static boolean validatePart(String part, boolean isFinalPart) {
    ...
    if (DASH_MATCHER.matches(part.charAt(0)) || DASH_MATCHER
  ...
  public InternetDomainName registrySuffix() {
    return hasRegistrySuffix() ? ancestor(this.registrySuffixIndex) : null;
  }
  ...
  private static boolean matchesWildcardSuffixType(Optional<PublicSuffixType> desiredType, String domain) {
    ...
    return (pieces.size() == 2 && 
      matchesType(desiredType, 
        Optional.fromNullable(PublicSuffixPatterns.UNDER.get(pieces.get(1)))));
  }
