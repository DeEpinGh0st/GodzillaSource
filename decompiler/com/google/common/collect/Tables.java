...
public final class Tables
{
  ...
  public static <T, R, C, V, I extends Table<R, C, V>> Collector<T, ?, I> toTable(Function<? super T, ? extends R> rowFunction, Function<? super T, ? extends C> columnFunction, Function<? super T, ? extends V> valueFunction, Supplier<I> tableSupplier) {
    return toTable(rowFunction, columnFunction, valueFunction, (v1, v2) -> { throw new IllegalStateException("Conflicting values " + v1 + " and " + v2); }tableSupplier);
  }
    ...
    extends AbstractTable<C, R, V>
  {
    ...
    Spliterator<Table.Cell<C, R, V>> cellSpliterator() {
      return (Spliterator)CollectSpliterators.map(this.original.cellSet().spliterator(), (Function<? super Table.Cell<?, ?, ?>, ? extends Table.Cell<?, ?, ?>>)TRANSPOSE_CELL);
    }
  }
  ...
  private static class TransformedTable<R, C, V1, V2> extends AbstractTable<R, C, V2> {
    ...
    public V2 remove(Object rowKey, Object columnKey) {
      return contains(rowKey, columnKey) ? (V2)this.function
        .apply(this.fromTable.remove(rowKey, columnKey)) : null;
    }
      ...
      return new Function<Table.Cell<R, C, V1>, Table.Cell<R, C, V2>>()
        {
          public Table.Cell<R, C, V2> apply(Table.Cell<R, C, V1> cell) {
            return Tables.immutableCell(cell
                .getRowKey(), cell.getColumnKey(), (V2)Tables.TransformedTable.this.function.apply(cell.getValue()));
          }
        };
    }
    ...
    Spliterator<Table.Cell<R, C, V2>> cellSpliterator() {
      return CollectSpliterators.map(this.fromTable.cellSet().spliterator(), (Function<? super Table.Cell<R, C, V1>, ? extends Table.Cell<R, C, V2>>)cellFunction());
    }
