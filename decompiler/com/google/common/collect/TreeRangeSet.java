  ...
  implements Serializable
{
  ...
  public Set<Range<C>> asDescendingSetOfRanges() {
    ...
    return (result == null) ? (this
      .asDescendingSetOfRanges = new AsRanges(this.rangesByLowerBound.descendingMap().values())) : result;
  }
  ...
  private Range<C> rangeEnclosing(Range<C> range) {
    Preconditions.checkNotNull(range);
    Map.Entry<Cut<C>, Range<C>> floorEntry = this.rangesByLowerBound.floorEntry(range.lowerBound);
    return (floorEntry != null && ((Range)floorEntry.getValue()).encloses(range)) ? floorEntry
      .getValue() : null;
  }
    ...
    extends AbstractNavigableMap<Cut<C>, Range<C>>
  {
    ...
    Iterator<Map.Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
      ...
      if (backingItr.hasNext() && this.upperBoundWindow.upperBound
        .isLessThan(((Range)backingItr.peek()).upperBound)) {
        backingItr.next();
      }
      return new AbstractIterator<Map.Entry<Cut<C>, Range<C>>>()
        {
          protected Map.Entry<Cut<C>, Range<C>> computeNext() {
            ...
            Range<C> range = backingItr.next();
            return TreeRangeSet.RangesByUpperBound.this.upperBoundWindow.lowerBound.isLessThan(range.upperBound) ? 
              Maps.<Cut<C>, Range<C>>immutableEntry(range.upperBound, range) : 
              endOfData();
          }
        };
    }
    ...
    public boolean isEmpty() {
      return this.upperBoundWindow.equals(Range.all()) ? this.rangesByLowerBound
        .isEmpty() : (
    ...
    extends AbstractNavigableMap<Cut<C>, Range<C>>
  {
    ...
    Iterator<Map.Entry<Cut<C>, Range<C>>> entryIterator() {
      ...
      if (this.complementLowerBoundWindow.contains((Cut)Cut.belowAll()) && (
        !positiveItr.hasNext() || ((Range)positiveItr.peek()).lowerBound != Cut.belowAll())) {
      ...
      return new AbstractIterator<Map.Entry<Cut<C>, Range<C>>>() {
          ...
          protected Map.Entry<Cut<C>, Range<C>> computeNext() {
            ...
            if (TreeRangeSet.ComplementRangesByLowerBound.this.complementLowerBoundWindow.upperBound.isLessThan(this.nextComplementRangeLowerBound) || this.nextComplementRangeLowerBound == 
              Cut.aboveAll()) {
              return endOfData();
            }
    ...
    Iterator<Map.Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
      ...
      final PeekingIterator<Range<C>> positiveItr = Iterators.peekingIterator(this.positiveRangesByUpperBound
      ...
      else { if (!this.complementLowerBoundWindow.contains((Cut)Cut.belowAll()) || this.positiveRangesByLowerBound
      ...
      final Cut<C> firstComplementRangeUpperBound = (Cut<C>)MoreObjects.firstNonNull(cut, Cut.aboveAll());
      return new AbstractIterator<Map.Entry<Cut<C>, Range<C>>>() {
    ...
    extends AbstractNavigableMap<Cut<C>, Range<C>>
  {
    ...
    private NavigableMap<Cut<C>, Range<C>> subMap(Range<Cut<C>> window) {
      ...
      return new SubRangeSetRangesByLowerBound(this.lowerBoundWindow
          .intersection(window), this.restriction, this.rangesByLowerBound);
    }
    ...
    public Range<C> get(Object key) {
      if (key instanceof Cut) {
        ...
        try {
          Cut<C> cut = (Cut<C>)key;
          if (!this.lowerBoundWindow.contains(cut) || cut
            .compareTo(this.restriction.lowerBound) < 0 || cut
            .compareTo(this.restriction.upperBound) >= 0)
            return null; 
  ...
  private final class SubRangeSet extends TreeRangeSet<C> {
    ...
    SubRangeSet(Range<C> restriction) {
      super(new TreeRangeSet.SubRangeSetRangesByLowerBound<>(
            
            Range.all(), restriction, TreeRangeSet.this.rangesByLowerBound, null));
    ...
    public void add(Range<C> rangeToAdd) {
      Preconditions.checkArgument(this.restriction
          .encloses(rangeToAdd), "Cannot add range %s to subRangeSet(%s)", rangeToAdd, this.restriction);
