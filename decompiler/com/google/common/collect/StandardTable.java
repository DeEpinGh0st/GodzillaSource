  ...
  implements Serializable
{
  ...
  class Row extends Maps.IteratorBasedAbstractMap<C, V> { final R rowKey;
    ...
    Map<C, V> backingRowMap() {
      return (this.backingRowMap == null || (this.backingRowMap.isEmpty() && StandardTable.this.backingMap.containsKey(this.rowKey))) ? (this
        .backingRowMap = computeBackingRowMap()) : this.backingRowMap;
    }
      ...
      return new Iterator<Map.Entry<C, V>>()
        {
          ...
          public Map.Entry<C, V> next() {
            return StandardTable.Row.this.wrapEntry(iterator.next());
          }
      ...
      protected Map.Entry<R, V> computeNext() {
        ...
        { final Map.Entry<R, Map<C, V>> entry = this.iterator.next();
          if (((Map)entry.getValue()).containsKey(StandardTable.Column.this.columnKey))
          {
            ...
            return new EntryImpl(); }  }  class EntryImpl extends AbstractMapEntry<R, V> {
    ...
    protected C computeNext() {
      while (true) {
        while (this.entryIterator.hasNext()) {
          ...
          this.entryIterator = ((Map<C, V>)this.mapIterator.next()).entrySet().iterator(); continue;
        }  break;
      }  return endOfData();
    }
