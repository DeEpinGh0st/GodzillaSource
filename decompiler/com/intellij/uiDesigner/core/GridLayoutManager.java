  ...
  extends AbstractLayout
{
  ...
  public GridLayoutManager(int rowCount, int columnCount) {
    ...
    this.myRowStretches = new int[rowCount]; int i;
  ...
  private static boolean shouldAddGapAfterCell(DimensionInfo info, int cellIndex) {
    ...
    int indexOfNextNotEmpty = -1; int i;
  ...
  public void layoutContainer(Container container) {
    ...
    if ((skipLayout & 0x1) == 0) {
      ...
      for (int j = 0; j < heights.length; j++) {
        this.myYs[j] = y;
        this.myHeights[j] = heights[j];
        y += heights[j];
        if (shouldAddGapAfterCell(verticalInfo, j)) {
    ...
    if ((skipLayout & 0x2) == 0) {
      ...
      for (int j = 0; j < widths.length; j++) {
        this.myXs[j] = x;
        this.myWidths[j] = widths[j];
        x += widths[j];
        if (shouldAddGapAfterCell(horizontalInfo, j)) {
  ...
  private int checkSetSizesFromParent(Container container, Insets insets) {
    ...
    if (parentGridLayout != null && parentGridConstraints.isUseParentLayout()) {
      if (this.myRowStretches.length == parentGridConstraints.getRowSpan()) {
        ...
        this.myHeights[this.myRowStretches.length - 1] = this.myHeights[this.myRowStretches.length - 1] - insets.bottom + this.myMargin.bottom;
        skipLayout |= 0x1;
      } 
      if (this.myColumnStretches.length == parentGridConstraints.getColSpan()) {
        ...
        this.myWidths[this.myColumnStretches.length - 1] = this.myWidths[this.myColumnStretches.length - 1] - insets.right + this.myMargin.right;
        skipLayout |= 0x2;
      } 
    } 
    return skipLayout;
  ...
  private int[] getMinOrPrefSizes(DimensionInfo info, boolean min) {
    ...
    for (int j = info.getComponentCount() - 1; j >= 0; j--) {
      ...
      int span = info.getSpan(j);
      int cell = info.getCell(j);
  ...
  private static void distribute(boolean[] higherPriorityCells, DimensionInfo info, int toDistribute, int[] widths) {
    ...
    for (int j = 0; j < info.getCellCount(); j++) {
      if (higherPriorityCells[j]) {
        ...
        int addon = toDistributeFrozen * info.getStretch(j) / stretches;
        widths[j] = widths[j] + addon;
    ...
    if (checkPrefs) {
      ...
      for (int i = 0; i < allowedCells.length; i++) {
        if (allowedCells[i])
        {
          ...
          if (!isCellEmpty(info, i) && prefs[i] > widths[i]) {
  ...
  public int[] getHorizontalGridLines() {
    return getGridLines(this.myYs, this.myHeights);
  }
  
  public int[] getVerticalGridLines() {
    return getGridLines(this.myXs, this.myWidths);
  }
