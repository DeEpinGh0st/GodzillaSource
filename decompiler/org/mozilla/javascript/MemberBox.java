      ...
      if (!Modifier.isPublic(c.getModifiers())) {
        ...
        for (int i = 0, N = intfs.length; i != N; i++) {
          ...
          if (Modifier.isPublic(intf.getModifiers())) {
            ...
            try { return intf.getMethod(name, params); }
        ...
        while (true) {
          c = c.getSuperclass();
          if (c == null)
            break;  if (Modifier.isPublic(c.getModifiers())) {
            
            try { Method m = c.getMethod(name, params);
  ...
  private static void writeParameters(ObjectOutputStream out, Class<?>[] parms) throws IOException {
    ...
    for (int i = 0; i < parms.length; i++) {
      ...
      if (!primitive) {
        ...
        int j = 0; while (true) { if (j < primitives.length) {
            if (parm.equals(primitives[j])) {
              out.writeByte(j); break;
            }  j++;
            continue;
          } 
