  ...
  implements DataBuffer
{
  ...
  public DefaultDataBuffer readPosition(int readPosition) {
    assertIndex((readPosition >= 0), "'readPosition' %d must be >= 0", new Object[] { Integer.valueOf(readPosition) });
    assertIndex((readPosition <= this.writePosition), "'readPosition' %d must be <= %d", new Object[] {
          Integer.valueOf(readPosition), Integer.valueOf(this.writePosition) });
  ...
  public DefaultDataBuffer writePosition(int writePosition) {
    assertIndex((writePosition >= this.readPosition), "'writePosition' %d must be >= %d", new Object[] {
          Integer.valueOf(writePosition), Integer.valueOf(this.readPosition) });
    assertIndex((writePosition <= this.capacity), "'writePosition' %d must be <= %d", new Object[] {
          Integer.valueOf(writePosition), Integer.valueOf(this.capacity) });
  ...
  public DefaultDataBuffer capacity(int newCapacity) {
    if (newCapacity <= 0) {
      throw new IllegalArgumentException(String.format("'newCapacity' %d must be higher than 0", new Object[] { Integer.valueOf(newCapacity) }));
    }
  ...
  public byte getByte(int index) {
    assertIndex((index >= 0), "index %d must be >= 0", new Object[] { Integer.valueOf(index) });
    assertIndex((index <= this.writePosition - 1), "index %d must be <= %d", new Object[] { Integer.valueOf(index), Integer.valueOf(this.writePosition - 1) });
    return this.byteBuffer.get(index);
  }

  
  public byte read() {
    assertIndex((this.readPosition <= this.writePosition - 1), "readPosition %d must be <= %d", new Object[] {
          Integer.valueOf(this.readPosition), Integer.valueOf(this.writePosition - 1) });
  ...
  public DefaultDataBuffer read(byte[] destination, int offset, int length) {
    Assert.notNull(destination, "Byte array must not be null");
    assertIndex((this.readPosition <= this.writePosition - length), "readPosition %d and length %d should be smaller than writePosition %d", new Object[] {
          
          Integer.valueOf(this.readPosition), Integer.valueOf(length), Integer.valueOf(this.writePosition)
        });
  ...
  public DefaultDataBuffer write(DataBuffer... buffers) {
    if (!ObjectUtils.isEmpty((Object[])buffers)) {
      write((ByteBuffer[])Arrays.<DataBuffer>stream(buffers).map(DataBuffer::asByteBuffer).toArray(x$0 -> new ByteBuffer[x$0]));
    }
    return this;
  }
  ...
  public DefaultDataBuffer write(ByteBuffer... buffers) {
    if (!ObjectUtils.isEmpty((Object[])buffers)) {
      int capacity = Arrays.<ByteBuffer>stream(buffers).mapToInt(Buffer::remaining).sum();
      ensureCapacity(capacity);
      Arrays.<ByteBuffer>stream(buffers).forEach(this::write);
    } 
    return this;
  }
  ...
  private int calculateCapacity(int neededCapacity) {
    ...
    if (neededCapacity > 4194304) {
      int i = neededCapacity / 4194304 * 4194304;
      if (i > 2143289343) {
        i = Integer.MAX_VALUE;
      } else {
    ...
    return Math.min(newCapacity, 2147483647);
  }
  ...
  public boolean equals(@Nullable Object other) {
    ...
    return (this.readPosition == otherBuffer.readPosition && this.writePosition == otherBuffer.writePosition && this.byteBuffer
      
      .equals(otherBuffer.byteBuffer));
  }
  ...
  public String toString() {
    return String.format("DefaultDataBuffer (r: %d, w: %d, c: %d)", new Object[] {
          Integer.valueOf(this.readPosition), Integer.valueOf(this.writePosition), Integer.valueOf(this.capacity)
        });
  }
  
  private void checkIndex(int index, int length) {
    assertIndex((index >= 0), "index %d must be >= 0", new Object[] { Integer.valueOf(index) });
    assertIndex((length >= 0), "length %d must be >= 0", new Object[] { Integer.valueOf(index) });
    assertIndex((index <= this.capacity), "index %d must be <= %d", new Object[] { Integer.valueOf(index), Integer.valueOf(this.capacity) });
    assertIndex((length <= this.capacity), "length %d must be <= %d", new Object[] { Integer.valueOf(index), Integer.valueOf(this.capacity) });
  }
