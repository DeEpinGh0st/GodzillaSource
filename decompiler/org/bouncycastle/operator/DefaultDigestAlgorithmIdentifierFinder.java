...
public class DefaultDigestAlgorithmIdentifierFinder implements DigestAlgorithmIdentifierFinder {
  ...
  private static Map digestNameToOids = new HashMap<Object, Object>();
  ...
  public AlgorithmIdentifier find(AlgorithmIdentifier paramAlgorithmIdentifier) {
    ...
    if (paramAlgorithmIdentifier.getAlgorithm().equals(PKCSObjectIdentifiers.id_RSASSA_PSS)) {
      algorithmIdentifier = RSASSAPSSparams.getInstance(paramAlgorithmIdentifier.getParameters()).getHashAlgorithm();
    } else {
      algorithmIdentifier = new AlgorithmIdentifier((ASN1ObjectIdentifier)digestOids.get(paramAlgorithmIdentifier.getAlgorithm()), (ASN1Encodable)DERNull.INSTANCE);
    } 
    return algorithmIdentifier;
  }
  
  public AlgorithmIdentifier find(String paramString) {
    return new AlgorithmIdentifier((ASN1ObjectIdentifier)digestNameToOids.get(paramString), (ASN1Encodable)DERNull.INSTANCE);
  }
