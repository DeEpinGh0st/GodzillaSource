...
public class PeLoader
{
  ...
  public static byte[] peToShellcode(byte[] peBuffer, StringBuilder logBuffer) throws Exception {
    PE pe = PEParser.parse(new ByteArrayInputStream(peBuffer));
    ...
    for (int i = 0; i < pe.getSectionTable().getNumberOfSections(); i++) {
      ...
      if (section != null) {
        ...
        System.arraycopy(sectionData, 0, _peBuffer, index, sectionData.length);
      } 
    } 
    ...
    if (pe.getOptionalHeader().getDataDirectory(IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR).getVirtualAddress() != 0) {
    ...
    String stubName = String.format("assets/stub%d.bin", new Object[] { Integer.valueOf(pe.is64() ? 64 : 32) });
    ...
    byte[] newExeBuffer = new byte[peBuffer.length + stub.length];
    ...
    String redir_codeHex = "4D5A4552E8000000005B4883EB09534881C3" + functions.byteArrayToHex(functions.intToBytes(peBuffer.length)) + "FFD3c3";


    
    byte[] redir_code = functions.hexToByte(redir_codeHex);
    
    System.arraycopy(peBuffer, 0, newExeBuffer, 0, peBuffer.length);
    
    System.arraycopy(stub, 0, newExeBuffer, peBuffer.length, stub.length);
    
    System.arraycopy(redir_code, 0, newExeBuffer, 0, redir_code.length);

    
    for (int j = 0; j < pe.getSectionTable().getNumberOfSections(); j++) {
      int secPtr = pe.getDosHeader().getAddressOfNewExeHeader() + (pe.is64() ? SIZE_64_IMAGE_NT_HEADERS : SIZE_86_IMAGE_NT_HEADERS) + SIZE_IMAGE_SECTION_HEADER * j;
      SectionHeader sectionHeader = pe.getSectionTable().getHeader(j);
      sectionHeader.setVirtualSize(get_virtual_sec_size(pe, j));
