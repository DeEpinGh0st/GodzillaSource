...
@PluginAnnotation(payloadName = "CShapDynamicPayload", Name = "ShapWeb", DisplayName = "SharpWeb")
public class SharpWeb
  implements Plugin
{
    ...
    this.splitPane.addComponentListener(new ComponentAdapter()
        {
          public void componentResized(ComponentEvent e) {
            SharpWeb.this.splitPane.setDividerLocation(0.15D);
          }
        });
  ...
  private void runButtonClick(ActionEvent actionEvent) {
    byte[] result = this.payload.evalFunc("SharpWeb.Run", "run", new ReqParameter());
