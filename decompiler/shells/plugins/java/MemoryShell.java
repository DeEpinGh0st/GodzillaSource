  ...
  implements Plugin
{
  ...
  private final JPanel panel = new JPanel(new GridBagLayout()); private final JLabel urlLabel; private final JLabel passwordLabel; private final JLabel payloadLabel; private final JLabel secretKeyLabel; private final JTextField urlTextField;
  ...
  private void runButtonClick(ActionEvent actionEvent) {
    try {
      ...
      if (secretKey.length() > 0 && pattern.length() > 0 && password.length() > 0) {
        ...
        String className = String.format("x.%s", new Object[] { shellName });
        InputStream inputStream = getClass().getResourceAsStream(String.format("assets/%s.classs", new Object[] { shellName }));
