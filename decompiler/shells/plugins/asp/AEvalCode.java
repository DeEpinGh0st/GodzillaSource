  ...
  implements Plugin
{
  ...
  private final JButton runButton = new JButton("Run"); private static final String CLASS_NAME = "AEvalCode"; private boolean loadState;
  public AEvalCode() {
    ...
    pane1.setLeftComponent(new JScrollPane((Component)this.codeTextArea));
    pane1.setRightComponent(runButtonPanel);
    
    pane2.setOrientation(1);
    pane2.setLeftComponent(pane1);
    pane2.setRightComponent(new JScrollPane((Component)this.resultTextArea));
  ...
  private void Load() {
    if (!this.loadState) {
      try {
        InputStream inputStream = getClass().getResourceAsStream("assets/evalCode.asp");
